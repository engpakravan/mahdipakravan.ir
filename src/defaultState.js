module.exports = {
    Header : {
        HeaderJumborton : true , 
        Brand : ['وبسایت' , 'مهدی پاکروان'],
        programmerImage : 'https://raw.githubusercontent.com/engpakravan/mahdipakravan.ir/master/src/Images/Programmer.png' ,
        list : [
          {
              id : 1 , 
              name : 'خانه' ,
              href : "/home"
          },
          {
              id : 2 , 
              name : 'چطوری کار میکنیم ؟' ,
              href : "/howWorking"
          },
          {
              id : 3 , 
              name : 'توانایی ها' ,
              href : "/skills"
          },
          {
              id : 4 , 
              name : 'نمونه کار ها' ,
              href : "/portfoilo"
          },
          {
              id : 5 , 
              name : 'تماس با ما' ,
              href : "/contactMe"
          }
        ],
        Welcome : `<h2>سلام من <span class="text-red">مهدی</span> هستم !</h2>` ,
        what_can_i_do : `میتونم با <kbd>جاوااسکریپت</kbd> آرزو های شما رو بسازم !` , 
        header_long_text : `لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ و با استفاده از طراحان گرافیک است چاپگرها و متون بلکه روزنامه و مجله در ستون و سطرآنچنان که لازم استلورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ و با استفاده از طراحان گرافیک است چاپگرها و متون بلکه روزنامه و مجله در ستون و سطرآنچنان که لازم است`
    },
    Sections : {
        howWorking : {
            title : 'چطوری با هیسم پروژه میسازیم ؟' , 
            title_meta : 'یک وبلاگ باحال هم داریم که <kbd>RealTime</kbd> هم هستش !' ,
            elements : [
                {
                    //TODO Get it From Server
                    decoration : "img_right" , 
                    key : '1' , 
                    stepText : 'مرحله اول' , 
                    hyperText : 'در مورد کسب و کارتون صحبت میکنیم !' ,
                    bodyText : 'لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از <kbd class="bg-red">صنعت چاپ</kbd> و با استفاده از طراحان گرافیک است چاپگرها و متون بلکه روزنامه و مجله در ستون و سطرآنچنان که لازم است و برای شرایط فعلی تکنولوژی مورد نیاز و کاربردهای متنوع با هدف بهبود ابزارهای کاربردی می باشدلورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ و با استفاده از طراحان گرافیک است چاپگرها و متون بلکه روزنامه و مجله در ستون و سطرآنچنان که لازم است و برای شرایط فعلی تکنولوژی مورد نیاز و کاربردهای متنوع با <kbd class="bg-red">هدف بهبود</kbd> ابزارهای کاربردی می باشد',
                    imageSource : 'https://roocket.ir/public/images/2018/12/3/discuss.png'
                } ,
                {
                    //TODO Get it From Server
                    decoration : "img_left" , 
                    key : '2' , 
                    stepText : 'مرحله دوم' , 
                    hyperText : 'در مورد کسب و کارتون صحبت میکنیم !' ,
                    bodyText : 'لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از <kbd class="bg-red">صنعت چاپ</kbd> و با استفاده از طراحان گرافیک است چاپگرها و متون بلکه روزنامه و مجله در ستون و سطرآنچنان که لازم است و برای شرایط فعلی تکنولوژی مورد نیاز و کاربردهای متنوع با هدف بهبود ابزارهای کاربردی می باشدلورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ و با استفاده از طراحان گرافیک است چاپگرها و متون بلکه روزنامه و مجله در ستون و سطرآنچنان که لازم است و برای شرایط فعلی تکنولوژی مورد نیاز و کاربردهای متنوع با <kbd class="bg-red">هدف بهبود</kbd> ابزارهای کاربردی می باشد',
                    imageSource : 'https://roocket.ir/public/images/2018/12/3/articles.png'
                },
                {
                    //TODO Get it From Server
                    decoration : "img_right" , 
                    key : '3' , 
                    stepText : 'مرحله سوم' , 
                    hyperText : 'در مورد کسب و کارتون صحبت میکنیم !' ,
                    bodyText : 'لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از <kbd class="bg-red">صنعت چاپ</kbd> و با استفاده از طراحان گرافیک است چاپگرها و متون بلکه روزنامه و مجله در ستون و سطرآنچنان که لازم است و برای شرایط فعلی تکنولوژی مورد نیاز و کاربردهای متنوع با هدف بهبود ابزارهای کاربردی می باشدلورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ و با استفاده از طراحان گرافیک است چاپگرها و متون بلکه روزنامه و مجله در ستون و سطرآنچنان که لازم است و برای شرایط فعلی تکنولوژی مورد نیاز و کاربردهای متنوع با <kbd class="bg-red">هدف بهبود</kbd> ابزارهای کاربردی می باشد',
                    imageSource : 'https://roocket.ir/public/images/2018/12/3/online-learning.png'
                },
                {
                    //TODO Get it From Server
                    decoration : "img_left" , 
                    key : '4' , 
                    stepText : 'مرحله چهارم' , 
                    hyperText : 'برای همیشه پشتیبانیتون میکنیم !' ,
                    bodyText : 'لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از <kbd class="bg-red">صنعت چاپ</kbd> و با استفاده از طراحان گرافیک است چاپگرها و متون بلکه روزنامه و مجله در ستون و سطرآنچنان که لازم است و برای شرایط فعلی تکنولوژی مورد نیاز و کاربردهای متنوع با هدف بهبود ابزارهای کاربردی می باشدلورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ و با استفاده از طراحان گرافیک است چاپگرها و متون بلکه روزنامه و مجله در ستون و سطرآنچنان که لازم است و برای شرایط فعلی تکنولوژی مورد نیاز و کاربردهای متنوع با <kbd class="bg-red">هدف بهبود</kbd> ابزارهای کاربردی می باشد',
                    imageSource : 'https://roocket.ir/public/images/2018/12/3/icon-2.png'
                }
            ]
        },
        skills : {
            title : 'چه توانایی هایی دارم ؟ ' , 
            title_meta : 'یک وبلاگ باحال هم داریم که <kbd>RealTime</kbd> هم هستش !',
            elements : [
                {
                    skillsKey: 'skills_1' , 
                    skillsTitle : 'برنامه نویسی Back_End' , 
                    elements : [
                        {
                            name : 'nodejs' , 
                            percent : '20'
                        },
                        {
                            name : 'GraphQL' , 
                            percent : '20'
                        },
                        {
                            name : 'MongoDB' , 
                            percent : '20'
                        },
                        {
                            name : 'Redis' , 
                            percent : '20'
                        },
                        {
                            name : 'MYSQL' , 
                            percent : '20'
                        }
                    ]
                },
                {
                    skillsKey: 'skills_2' , 
                    skillsTitle : 'برنامه نویسی Front_End' , 
                    elements : [
                        {
                            name : 'HTML' , 
                            percent : '100'
                        },
                        {
                            name : 'React.js' , 
                            percent : '40'
                        },
                        {
                            name : 'MongoDB' , 
                            percent : '20'
                        },
                        {
                            name : 'Redis' , 
                            percent : '20'
                        },
                        {
                            name : 'MYSQL' , 
                            percent : '20'
                        }
                    ]
                },
                {
                    skillsKey: 'skills_3' , 
                    skillsTitle : 'متفرقه' , 
                    elements : [
                        {
                            name : 'nodejs' , 
                            percent : '20'
                        },
                        {
                            name : 'GraphQL' , 
                            percent : '20'
                        },
                        {
                            name : 'MongoDB' , 
                            percent : '20'
                        },
                        {
                            name : 'Redis' , 
                            percent : '20'
                        },
                        {
                            name : 'MYSQL' , 
                            percent : '20'
                        }
                    ]
                }
            ]
        },
        portfoilo : {
            title : 'نمونه کار های من',
            title_meta : 'یک وبلاگ باحال هم داریم که <kbd>RealTime</kbd> هم هستش !', 
            Constant_data : {
                myJobText : 'وظیفه ی من' , 
                usesTechnologyText : 'تکنولوژی های استفاده شده : ' ,
                descText : 'توضیحات بیشتر : '
            },
            elements : [
                {
                    order : 1 , 
                    name : 'اپلیکیشن ماین تاکسی' , 
                    job : 'توسعه ی سمت سرور (Back End) اپلیکیشن' , 
                    technology : 'Nodejs - Mysql - Redis - MongoDB - Bootstrap - SocketIO - FireBase' , 
                    desc : 'ماین، سامانه هوشمند حمل‌ونقل، پل ارتباطی میان کاربر مسافر و کاربر راننده است و راهکاری سریع و ساده برای درخواست خودرو و موتورسیکلت به شمار می‌رود. با درخواست خودرو یا موتورسیکلت در این اپلیکیشن، پس از مشخص کردن مبدا و مقصد، نزدیک‌ترین ماین به نشانی شما می‌آید. با ماین سفری ایمن، راحت و مقرون‌به‌صرفه را تجربه خواهید کرد.',
                    decoration : 'img_left' ,
                    imgSrc : "http://minetaxi.ir/assets/images/mine/faq.png"
                } ,
                {
                    order : 2 , 
                    name : 'اپلیکیشن ماین تاکسی' , 
                    job : 'توسعه ی سمت سرور (Back End) اپلیکیشن' , 
                    technology : 'Nodejs - Mysql - Redis - MongoDB - Bootstrap - SocketIO - FireBase' , 
                    desc : 'ماین، سامانه هوشمند حمل‌ونقل، پل ارتباطی میان کاربر مسافر و کاربر راننده است و راهکاری سریع و ساده برای درخواست خودرو و موتورسیکلت به شمار می‌رود. با درخواست خودرو یا موتورسیکلت در این اپلیکیشن، پس از مشخص کردن مبدا و مقصد، نزدیک‌ترین ماین به نشانی شما می‌آید. با ماین سفری ایمن، راحت و مقرون‌به‌صرفه را تجربه خواهید کرد.',
                    decoration : 'img_right' ,
                    imgSrc : "http://minetaxi.ir/assets/images/mine/faq.png"
                }
            ]
        }
    }
}